...
export async function POST(
  ...
  try {
    ...
    if (file.size > MAX_SIZE) {
      return failure(UPLOAD_ERRORS.IMAGE_TOO_LARGE.code, `${UPLOAD_ERRORS.IMAGE_TOO_LARGE.message} Limite: 2MB.`, 400, [
        { field: UPLOAD_ERRORS.IMAGE_TOO_LARGE.field, message: `${UPLOAD_ERRORS.IMAGE_TOO_LARGE.message} Limite: 2MB.` },
      ])
    }
    ...
    return success({ logoUrl: upload.publicUrl })
  } catch (err) {
